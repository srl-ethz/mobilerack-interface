...
public:
    ...
    QualisysClient(int numframes, std::vector<int> cameraIDs = {}, bool nan_if_missed = false) : cameraIDs(cameraIDs), nan_if_missed(nan_if_missed){
...
//     QualisysClient(int numframes, std::vector<int> cameraIDs, bool nan_if_missed) : cameraIDs(cameraIDs), nan_if_missed(nan_if_missed){
//         frames.resize(numframes); // for base + each segment
//         images.resize(cameraIDs.size());
//         connect_and_setup();
//         motiontrack_thread = std::thread(&QualisysClient::motiontrack_loop, this);
