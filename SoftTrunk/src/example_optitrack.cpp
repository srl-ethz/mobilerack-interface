/*
 * Based on SimpleExample.cpp provided as example in NatNetLinux.
 */
...
void readOpts(int argc, char *argv[]) {
  ...
  Globals::serverAddress =
      inet_addr(vm["server-addr"].as<std::string>().c_str());
}
...
void printFrames(FrameListener &frameListener) {
  ...
  while (Globals::run) {
    while (true) {
      ...
      /*
      MocapFrame is described in NatNet.h
      frame.frameNum(): frame ID
      frame.rigidBodies(): vector of "RigidBody"s

      RigidBody is defined in NatNet.h
...
void timeStats(FrameListener &frameListener, const float diffMin_ms = 0.5,
               const float diffMax_ms = 7.0, const int bins = 100) {
  size_t hist[bins];
  float diff_ms;
  int bin;
  ...
  while (Globals::run) {
    while (true) {
      ...
      diff_ms = std::abs((static_cast<float>(current.tv_sec) -
                          static_cast<float>(prev.tv_sec)) *
                             1000.f +
                         (static_cast<float>(current.tv_nsec) -
                          static_cast<float>(prev.tv_nsec)) /
                             1000000.f);
  ...
  for (bin = 0; bin < bins; ++bin)
    std::cout << diffMin_ms + (diffMax_ms - diffMin_ms) * (0.5f + bin) / bins
              << ", " << hist[bin] << std::endl;
}
