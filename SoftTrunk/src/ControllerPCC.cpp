...
ControllerPCC::ControllerPCC(AugmentedRigidArm* augmentedRigidArm, SoftArm* softArm) : ara(augmentedRigidArm), sa(softArm){
    if (USE_PID_CURVATURE_CONTROL){
        ...
        miniPIDs.push_back(MiniPID(0.9,0,0.001/CONTROL_PERIOD)); // PID for phi0
        miniPIDs.push_back(MiniPID(0.8,0,0.001/CONTROL_PERIOD)); // PID for phi1
        miniPIDs.push_back(MiniPID(0.8,0,0)); // PID for phi2
    }
}
